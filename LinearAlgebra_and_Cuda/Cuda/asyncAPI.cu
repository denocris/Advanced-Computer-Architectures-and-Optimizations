...
int main(int argc, char *argv[])
{
    ...
    cudaEventCreateWithFlags(&stop, cudaEventDisableTiming); // the flag "cudaEventDisableTiming" Specifies that the created event does not need to record timing data.

    // asynchronously issue work to the GPU (all to stream 0)
    CHECK(cudaMemcpyAsync(d_a, h_a, nbytes, cudaMemcpyHostToDevice)); // is asynchronous with respect to the host, so the call may return before the copy is complete
    //CHECK(cudaMemcpy(d_a, h_a, nbytes, cudaMemcpyHostToDevice)); // Try this!! Blocking
    kernel<<<grid, block>>>(d_a, value);
    CHECK(cudaMemcpyAsync(h_a, d_a, nbytes, cudaMemcpyDeviceToHost));
    //CHECK(cudaMemcpy(h_a, d_a, nbytes, cudaMemcpyDeviceToHost)); // Try this!! Blocking
